...
export const useAnonymousLoginMutation = (
  options?: t.MutationOptions<t.TLoginResponse, { fingerprint: any }, unknown, unknown>,
): UseMutationResult<t.TLoginResponse, unknown, { fingerprint: any }, unknown> => {
  const queryClient = useQueryClient();
  const clearStates = useClearStates();
  const resetDefaultPreset = useResetRecoilState(store.defaultPreset);
  const setQueriesEnabled = useSetRecoilState<boolean>(store.queriesEnabled);
  return useMutation([MutationKeys.anonymousLogin], {
    mutationFn: (payload: { fingerprint: any }) => dataService.anonymousLogin(payload),
    ...(options || {}),
    onMutate: (vars) => {
      resetDefaultPreset();
      clearStates();
